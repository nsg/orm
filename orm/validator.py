...
def get_schema(yml_file, yml_doc, schema_type='rules'):
    ...
    if not schema_version:
        raise ORMSchemaException('ORM doc missing schema_version ('
                                 + yml_file + ')')
    schema_file = orm_schemas[schema_type].get(schema_version, None)
    if not schema_file:
        raise ORMSchemaException('ORM doc using unknown schema_version. '
                                 'Found \'' + schema_version + '\'. '
                                 'Supports ' + str(list(orm_schemas.keys()))
                                 + ' (' + yml_file + ')')
...
uri_all_components_validator = validators.Validator(
).require_presence_of(
...
def format_check_uri_path(instance):
    uri = uri_reference('http://example.com/{}?param=value#fragment'
                        .format(instance))
    try:
...
regex_unix_user_or_group = re.compile(
...
rfc1123_hostname = (r'(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*'
...
regex_hostport = re.compile('^' + rfc1123_hostname
                            + '(' + parser.port_delim + reg_port + ')?$')
...
orm_origin = (r'^(' + rfc2396_scheme + parser.scheme_delim + ')?'
              + rfc1123_hostname
              + r'(' + parser.port_delim + reg_port + ')?$')
...
def is_ascii_letter_range(start_char, end_char):
    return (start_char in string.ascii_letters
            and end_char in string.ascii_letters
            and ((start_char.isupper() and end_char.isupper()
        ...
        if char == '[' and prev_char != '\\':
            ...
            if instance[i + 1] == '-' and char != '\\': # character range
                ...
                if is_ascii_letter_range(char, end_char):
                    ...
                    new_inst += (char.lower() + '-' + end_char.lower()
                                 + char.upper() + '-' + end_char.upper())
                else:
...
def fsm_action_task(action, fsm1, fsm2):
    if action == 'and':
        ...
        raise ValidateRuleConstraintsException('Unknown fsm_action_task '
                                               'action: ' + action)
...
def get_match_path_fsm(match_tree, worker_pool=None):
    ...
    def handle_condition_list(data_list_in, op, negate):
        def divide_and_conquer(fsm_list, op):
            ...
            future = worker_pool.submit(fsm_action_task,
                                        op,
                                        fsm_result1,
                                        fsm_result2)
            return future
    ...
    def handle_match(src, fun, inp, negate):
        ...
        if fun == 'begins_with':
            ...
            raise ValidateRuleConstraintsException('Handling of '
                                                   'match function ' + str(fun)
                                                   + ' not implemented.')
        future = worker_pool.submit(fsm_parse_regex_task,
                                    regex,
                                    negate,
                                    alphabet)
        return future
        ...
        for rule in rules:
            ...
            if domain_default is not None:
                if not domain_default:
                    print()
                    print('ERROR: Using domain_default and setting it to false '
                          ' is not allowed')
                    print(rule['_orm_source_file']
                          + ' (' + rule['description'] + ')')
                    return False
                if domain_default_rule:
                    ...
                    print(domain_default_rule['_orm_source_file']
...
def validate_constraints_rule_collision(domain_rules, cache_path=None):
    ...
    for fsm_future in concurrent.futures.as_completed(fsm_futures):
        ...
        fsm_entry['fsm'] = fsm
        print('Generated FSM for '
              + fsm_entry['file'] + ': ' + fsm_entry['desc'])
    ...
    for i, fsm_one in enumerate(rule_fsm_list):
        ...
        while j < len(rule_fsm_list):
            ...
            if (fsm_one['domain'] == fsm_two['domain']):
                future = worker_pool.submit(fsm_collision,
                                            fsm_one['fsm'],
                                            fsm_two['fsm'])
...
def validate_rule_constraints(yml_files=None, cache_path=None):
    ...
    if not validate_constraints_domain_default(orm_docs['rules']):
        return False
    if not validate_constraints_rule_collision(orm_docs['rules'],
